...
const pushExporterJob = (event: Event, version?: string) => {
  return new PushImageJob(pushExporterJobName, event, version)
}
...
const pushGrafanaJob = (event: Event, version?: string) => {
  return new PushImageJob(pushGrafanaJobName, event, version)
}
...
events.on("brigade.sh/github", "cd:pipeline_requested", async event => {
  ...
  await new SerialGroup(
    new ConcurrentGroup(
      pushExporterJob(event, version),
      pushGrafanaJob(event, version)
    ),
